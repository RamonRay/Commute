...
Shader "MyShaders/Rim Light Shader"{
	...
	Properties{
		...
		_ReflectionOn("Reflection on(0 or 1)", Int) = 1
		_RefractionRate("Refraction Rate", Range(0,50)) = 0.3
		_RefractionOn("Refraction on(0 or 1)", Int) = 0
		_RimlightShineSpeed("Rimlight Shine Speed", Float) = 1.0
	}
		...
		Pass{
			...
			*/
			...
			fixed4 frag(v2f i) : SV_Target{
				...
				float AdjustedReflectionRate = sin(_Time.y * _RimlightShineSpeed)*4 + 4.6;
				float Fr = pow(1-adotNV, AdjustedReflectionRate);
				float Ft = pow(adotNV, _RefractionRate);

				fixed3 rim = saturate(_RimColor.rgb * (Fr*_ReflectionOn + Ft * _RefractionOn));

				return fixed4(ambient + (diffuse + specular ) * atten + rim, 1.0);
			}
